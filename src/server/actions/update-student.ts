...
export async function updateStudent(
  studentId: string,
  _prevState: UpdateStudentState | null,
  formData: FormData
  ...
  const [existingByIdentifier, existingByNumber, group] = await Promise.all([
    prisma.student.findFirst({
      where: { identifier, id: { not: studentId } },
    }),
    studentNumber != null
      ? prisma.student.findFirst({
          where: { studentNumber, id: { not: studentId } },
        })
  ...
  await prisma.$transaction(async (tx) => {
    ...
    const activeMembership = await tx.studentMembership.findFirst({
    ...
    if (beltLevelId && beltDate) {
      const latestBelt = await tx.studentBeltHistory.findFirst({
        where: { studentId },
        orderBy: [{ promotionDate: "desc" }, { createdAt: "desc" }],
      });
      const isSameAsLatest =
        latestBelt &&
        latestBelt.beltLevelId === beltLevelId &&
        latestBelt.promotionDate.toISOString().slice(0, 10) ===
          beltDate.toISOString().slice(0, 10);
      if (!latestBelt) {
        ...
        await tx.studentBeltHistory.update({
          where: { id: latestBelt.id },
          data: { certificateNumber: beltCertificateNumber || null },
        });
      }
    } else if (beltLevelId === "" || beltLevelId === null) {
      const latestBelt = await tx.studentBeltHistory.findFirst({
        where: { studentId },
        orderBy: { promotionDate: "desc" },
      });
      if (latestBelt) {
        await tx.studentBeltHistory.delete({
          where: { id: latestBelt.id },
        });
      }
    }
